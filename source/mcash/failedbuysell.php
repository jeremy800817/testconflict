...
if ('spot_acebuy' == $_SESSION['get_transaction_error_message_type']) {
    // Customer Sell
	$headmessage = $lang['UnsuccessfulSell'];
	if("Error (20007): Gold balance insufficient, unable to perform transaction." == $_SESSION['get_transaction_error_message']){
		$error_message = $lang['InsufficientGoldBalanceErr'];
	}else{
		$error_message = $_SESSION['get_transaction_error_message'];
	}
	
}else{
	// Customer Buy
    $headmessage = $lang['UnsuccessfulBuy'];
	if("Error (20007): Gold balance insufficient, unable to perform transaction." == $_SESSION['get_transaction_error_message']){
		$error_message = $lang['InsufficientGoldBalanceErr'];
	}else{
		$error_message = $_SESSION['get_transaction_error_message'];

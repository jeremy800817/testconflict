...
CASE
    WHEN `vw_order`.ord_type = "CompanyBuy" THEN `vw_mydisbursement`.dbm_gatewayrefno
    WHEN `vw_order`.ord_type = "CompanySell" THEN `vw_mypaymentdetail`.pdt_gatewayrefno
    ELSE `vw_mypaymentdetail`.pdt_gatewayrefno
END
as gtr_dbmpdtgatewayrefno,

CASE
    WHEN `vw_order`.ord_type = "CompanyBuy" THEN `vw_mydisbursement`.dbm_refno
    WHEN `vw_order`.ord_type = "CompanySell" THEN `vw_mypaymentdetail`.pdt_paymentrefno
...
CASE
    WHEN `vw_order`.ord_type = "CompanyBuy" THEN `vw_mydisbursement`.dbm_requestedon
    WHEN `vw_order`.ord_type = "CompanySell" THEN `vw_mypaymentdetail`.pdt_requestedon
...
CASE
    WHEN `vw_order`.ord_type = "CompanyBuy" THEN `vw_mydisbursement`.dbm_accountholdername
    WHEN `vw_order`.ord_type = "CompanySell" THEN `vw_mypaymentdetail`.pdt_accountholdername
    ELSE `vw_mypaymentdetail`.pdt_accountholdername
END
as gtr_dbmpdtaccountholdername,

CASE
    WHEN `vw_order`.ord_type = "CompanyBuy" THEN `vw_mydisbursement`.dbm_accountholdercode
    WHEN `vw_order`.ord_type = "CompanySell" THEN `vw_mypaymentdetail`.pdt_accountholdercode
    ELSE `vw_mypaymentdetail`.pdt_accountholdercode
END
as gtr_dbmpdtaccountholdercode,

CASE
    WHEN `vw_order`.ord_type = "CompanyBuy" THEN `vw_mydisbursement`.dbm_verifiedamount
    WHEN `vw_order`.ord_type = "CompanySell" THEN `vw_mypaymentdetail`.pdt_verifiedamount
    ELSE `vw_mypaymentdetail`.pdt_verifiedamount
END
as gtr_dbmpdtverifiedamount,


`vw_order`.ord_partnerid as gtr_ordpartnerid,
`vw_order`.ord_productname as gtr_ordproductname,
`vw_order`.ord_buyerid as gtr_ordbuyerid,
`vw_order`.ord_orderno as gtr_ordorderno,
`vw_order`.ord_type as gtr_ordtype,
`vw_order`.ord_price as gtr_ordprice,
`vw_order`.ord_xau as gtr_ordxau,
`vw_order`.ord_amount as gtr_ordamount,
`vw_order`.ord_fee as gtr_ordfee,
`vw_order`.ord_remarks as gtr_ordremarks,
`vw_order`.ord_isspot as gtr_ordisspot,
`vw_order`.ord_bookingon as gtr_ordbookingon,
`vw_order`.ord_confirmon as gtr_ordconfirmon,
`vw_order`.ord_cancelon as gtr_ordcancelon,
`vw_order`.ord_status as gtr_ordstatus,
...
LEFT JOIN `vw_order`
ON gtr.gtr_orderid = `vw_order`.ord_id

...
//websocketurl need to change the value later
var websocketurl = window.location.protocol == 'https:' ? 'wss://' : 'ws://'; websocketurl += window.location.hostname + '/index.php?hdl=pssubscribe&action=subscribesales&pdt=DG-999-9&code=INTLX.KGOLDAFF';
// var host_name = window.location.host;
// if (host_name.search('demo') >= 0 || host_name.search('uat') >= 0 ){
//     var websocketurl = window.location.protocol == 'https:' ? 'wss://' : 'ws://'; websocketurl += window.location.hostname + '/index.php?hdl=pssubscribe&action=subscribesales&pdt=DG-999-9&gopayz=1&code=INTLX.GOPAYZ';
// }else{
//     var websocketurl = window.location.protocol == 'https:' ? 'wss://' : 'ws://'; websocketurl += window.location.hostname + '/index.php?hdl=pssubscribe&action=subscribesales&pdt=DG-999-9&gopayz=1&code=INTLX.GOPAYZ';
// }
Ext.define('snap.store.kgoldaffiSalesPriceStream', {
	...
	alias: 'store.kgoldaffiSalesPriceStream',
	model: 'snap.model.SalesPriceStream',
	storeId: 'kgoldaffiSalesPriceStream',
	proxy: {
		type: 'websocket',
        storeId: 'kgoldaffiSalesPriceStream',
			...
			transform: {
				fn: function (data) {
                    ...
                    */
                    ...
                    acebuydesign = '<h2 style="text-align:center;text-transform: uppercase;">Ace Buy</h2>' +
                    '<br><div>' + 
                    acebuyprice + 
                   ...
                   aceselldesign = '<h2 style="text-align:center;text-transform: uppercase;">Ace Sell</h2>' +
                   '<br><div>' + 
                   acesellprice + 
                   '<h3 style="text-align:center;font-style: italic;">per gram</h3>' +
                   '</div>';
                ...
                */// Old color #404040
                ...
                // End of Ace Sell Price Confirmation
					...
					Ext.getCmp('kgoldaffisaleacesellprice').setValue(companysellppg);
					Ext.getCmp('kgoldaffisaleacebuyprice').setValue(companybuyppg);
                    Ext.getCmp('kgoldaffisaleorderuuid').setValue(data[0].uuid);
                    
                    prevbuyprice = companybuyppg;
                    prevsellprice = companysellppg;
                    
                    Ext.getCmp('kgoldaffispotsaleacebuy').setValue(acebuydesign);
                    Ext.getCmp('kgoldaffispotsaleacesell').setValue(aceselldesign);

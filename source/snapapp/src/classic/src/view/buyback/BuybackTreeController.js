Ext.define('snap.view.buyback.BuybackTreeController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.gridpanel-buybacktreecontroller',
    ...
    onRequestWindowSaveButton: (elemnt) => {
        // var additional_bloodtestitem = elemnt.lookupReferenceHolder('request_window').getView().getViewModel().data.blooditemmap
        

        // console.log(elemnt);return;
        lists = elemnt.lookupReferenceHolder('collectiontab').getView().getController().lookupReference('pocontainer').getStore().data.items
        ...
        salesmanid = elemnt.lookupReferenceHolder('collectiontab').getView().getController().lookupReference('salesperson').value;
        deliverydate = elemnt.lookupReferenceHolder('collectiontab').getView().getController().lookupReference('deliverydate').value;


        if (!salesmanid || !deliverydate){
            Ext.MessageBox.show({
                title: 'Create Buyback Logistic',
                msg: 'Failed, required field empty.',
        ...
        snap.getApplication().sendRequest({
            ...
            data: JSON.stringify(trans_po),
            salesmanid: salesmanid,
            deliverydate: deliverydate,
            partnerid: lists[0].data.partnerid
    ...
    onWindowCancel: function(elemnt){
        elemnt.up('window').destroy();
    }
});

Ext.define('snap.view.collection.SharedCollectionController', {
    extend: 'snap.view.gridpanel.BaseController',
    alias: 'controller.collection-sharedcollection',
    ...
    uploadcollectionshared: function(btn, formAction) {
        var me = this, selectedRecord,
            myView = this.getView();
        var sm = myView.getSelectionModel();
        

        var gridFormView = Ext.create(myView.formClass, Ext.apply(myView.uploadcollectionsharedform ? myView.uploadcollectionsharedform : {}, {
            formDialogButtons: [{
                xtype:'panel',
                flex:3
            },
    ...
    _uploadFile: function(btn) {
        ...
        form = btn.lookupController().lookupReference('grnsharedlist-form').getForm();
        transactionlisting = form.getFieldValues();
        if (form.isValid()) {
            if ( transactionlisting.grnsharedlist != null) {
                form.submit({
                    url: 'index.php?hdl=collection&action=uploadCollectionSHARED',
                    // url: 'index.php?hdl=tender&action=uploadTenderFile',
                    dataType: "json",
                    partner: myView.partnercode,
                    waitMsg: 'Uploading your '+myView.partnercode+' GRN list...',
    ...
    getPrintReport: function(btn){
        ...
        url = '?hdl=sharedcollection&action=exportExcel&header='+header+'&daterange='+daterange+'&partner='+myView.partnercode;

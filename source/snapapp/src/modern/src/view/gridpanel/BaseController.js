Ext.define('snap.view.gridpanel.BaseController', {
    ...
    onDelete: function(btn) {
        ...
        var myView = this.getView();
        var grid = myView.down('grid');   
        var selectedRecords = grid.getSelections();
              

        //var selectedRecords = sm.getSelection();
        if (selectedRecords.length == 0) {
            Ext.Msg.alert('Warning', 'Select a record first');
            return;
        } else /*if (selectedRecords.length > 0)*/ {
        	new Ext.Promise(function(fulfilled, rejected) {
        		Ext.Msg.confirm('Confirm', Ext.String.format(confirmMessage, selectedRecords.length), function(btn) {
                ...
                snap.getApplication().sendRequest({ hdl: grid.getStore().getModel().entityName.toLowerCase(), 
    ...
    ordernoColor: function (value, record, dataIndex, cell) {
        if (record.data.type == 'CompanySell'){
            cell.setStyle('color: #209474;')
        }
        if (record.data.type == 'CompanyBuy'){

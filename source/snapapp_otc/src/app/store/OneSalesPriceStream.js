...
var websocketurl = window.location.protocol == 'https:' ? 'wss://' : 'ws://'; websocketurl += window.location.hostname + '/streamprice?hdl=pssubscribe&action=subscribesales&pdt=DG-999-9&onecent=1&code=INTLX.ONECENT';
...
//     var websocketurl = window.location.protocol == 'https:' ? 'wss://' : 'ws://'; websocketurl += window.location.hostname + '/streamprice?hdl=pssubscribe&action=subscribesales&pdt=DG-999-9&onecent=1&code=INTLX.ONECENT';
// }
Ext.define('snap.store.OneSalesPriceStream', {
	...
	alias: 'store.OneSalesPriceStream',
	model: 'snap.model.SalesPriceStream',
	storeId: 'onesalesPriceStream',
	proxy: {
		type: 'websocket',
        storeId: 'onesalesPriceStream',
			...
			transform: {
                ...
                */// Old color #404040
                ...
                // End of Ace Sell Price Confirmation
					...
					Ext.getCmp('onesaleacesellprice').setValue(companysellppg);
					Ext.getCmp('onesaleacebuyprice').setValue(companybuyppg);
                    Ext.getCmp('onesaleorderuuid').setValue(data[0].uuid);
                    
                    prevbuyprice = companybuyppg;
                    prevsellprice = companysellppg;
                    
                    Ext.getCmp('onespotsaleacebuy').setValue(acebuydesign);
                    Ext.getCmp('onespotsaleacesell').setValue(aceselldesign);

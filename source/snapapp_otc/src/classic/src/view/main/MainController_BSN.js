 ...
 */
Ext.define('snap.view.main.MainController_BSN', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.main_BSN',
    init: function(view) {
        var me = this;
    ...
    onMainViewRender:function() {
        if (!window.location.hash) {
            if (snap.getApplication().direct != false){
                ...
                this.redirectTo("orderdashboardview_BSN");
            }
        }
    },
    ...
    initViewModel: function() {
        ...
        if (snap.getApplication().info.env == 'dev'){
            ...
            this.getViewModel().set('copyright', '<b>POC</b> - All Rights Reserved &copy; 2023 ACE Innovate Asia Berhad');
            this.getViewModel().set('version', '1.8.0');
            jQuery("title").prepend("POC-")
        }

        this.getViewModel().set('username', snap.getApplication().username);
        this.getViewModel().set('displayname', snap.getApplication().displayname);
        this.getViewModel().set('userid', snap.getApplication().userid);
        this.getViewModel().set('partnerid', snap.getApplication().partnerid);
    ...
    openWebSocketConnection: function () {
        ...
        webSocket = new WebSocket("wss://"+window.location.hostname+window.location.pathname+"index.php?hdl=pssubscribe&action=subscribenotification");

Ext.define('snap.view.myaccountholder.MyAccountHolderController', {
    ...
    onViewAccountHolder: function () {
        ...
        var partnerCode = PROJECTBASE;
        // set path
        path = 'my' + partnerCode.toLowerCase() + 'cifview';
    ...
    onSuspendAccountHolder: function (btn) {
        ...
        let win = new Ext.Window ({
            title:'Freeze Account Confirmation',
            layout:'form',
            width:530,
            closeAction:'close',
    ...
    getPrintReportFromSearch: function(btn){

        var myView = this.getView(),
        // grid header data
        header = [];
        partnerCode = myView.partnerCode ? myView.partnerCode : myView.partnercode;
        ...
        currentProxy = btn.up().up().up().up().up().getController().lookupReference('myaccountholdersearchresults').getStore().proxy.url;
        ...
        url = '?hdl=myaccountholderhandler&action=exportExcelAccounts&header='+header+'&daterange='+daterange+'&partnercode='+partnerCode+'&fromsearchresult='+true+carryOverParams;
        // url = Ext.urlEncode(url);
    ...
    printRegisterPDF: function(btn) {
        ...
        var url = 'index.php?hdl=myaccountholder&action=printRegisterPDF&accountholderid='+selectedRecords[0].data.id;
				Ext.Ajax.request({
					url: url,
					method: 'get',
					waitMsg: 'Processing',

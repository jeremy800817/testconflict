Ext.define('snap.view.myaccountholder.MyAccountHolderLoan', {
    extend: 'snap.view.gridpanel.Base',
    xtype: 'myaccountholderloanview',
    ...
    checkActionPermission: function (view, record) {
        ...
        /* Add Upload Button */
        var updateLoanBtn = Ext.ComponentQuery.query('#updateLoanBtn')[0];
        updateLoanBtn.disable();
        var updateLoanPermission = snap.getApplication().hasPermission('/root/'+ parCode +'/profile/updateLoan');

        // Allow only if Master Type is LOAN and Loan Status is No
        if (updateLoanPermission == true && selected && record.data.partnerparent == 1 && record.data.loanstatus == 0) {
            updateLoanBtn.enable();
        }

        /* Add Upload FTP Button */
        // var uploadLoanBtn = Ext.ComponentQuery.query('#uploadLoanBtn')[0];
        // uploadLoanBtn.disable();
        // var uploadLoanPermission = snap.getApplication().hasPermission('/root/'+ parCode +'/profile/updateLoan');

        // // Allow only if Master Type is LOAN and Loan Status is No
        // if (uploadLoanPermission == true && selected && record.data.partnerparent == 1 && record.data.loanstatus == 0) {
        //     uploadLoanBtn.enable();
        // }
    ...
    listeners: {
        afterrender: function () {
            ...
            myView = this;
            // Get Partner Id for the module
            snap.getApplication().sendRequest({
                hdl: 'myaccountholder', action: 'getPartnerId',
                partnercode : this.partnerCode,
            }, 'Fetching data from server....').then(
                function (data) {
                    if (data.success) {
                        myView.partnerId = data.partnerid;
                        myView.parent = data.parent;
                    }
                })
        },
    ...
    columns: [
        ...
        {
            text: 'Master Type', dataIndex: 'partnerparent', filter: { type: 'string' }, minWidth: 100, align: 'left',
            filter: {
                type: 'combo',
                store: [
                    ['0', 'Default'],
            ...
            renderer: function (value, rec) {
                ...
                else if (value == '1') return 'Loan';
                else if (value == '2') return 'Affiliate Member';
                else if (value == '3') return 'Public';
                else if (value == '4') return 'Affiliate Public';
                else return 'Unidentified';
            },
            // renderer: function (val, m, record) {
            //     // If PEP
            //     if (record.data.partnerparent) {
            //         if (record.data.partnerparent == 0) {
            //             // PEP Status Pending
            //             return '<span class="fas fa-spinner fa-spin x-color-warning"></span>';
            //         } else if (record.data.partnerparent == 1) {
            //             // PEP Status Passed
            //             return '<span class="fa fa-circle x-color-success"></span>';
            //         } else if (record.data.partnerparent == 2) {
            //             // PEP Status Failed
        ...
        { text: 'Loan Approved On', dataIndex: 'loanapprovedate', xtype: 'datecolumn', format: 'Y-m-d', filter: { type: 'date' }, inputType: 'hidden', minWidth: 100 },
        { text: 'Approved by', dataIndex: 'ach_loanapprovebyname', filter: { type: 'string' }, minWidth: 130, flex: 1 },
        {
    ...
    uploadcollectionposform: {
        ...
        formDialogTitle: "Loan Approval File",
                ...
                items: [
                    {
                        ...
                        items: [
                            
                            {
                                ...
                                flex:1,
                                xtype: 'displayfield',
                                value : "<p>&#9679; Minimum 1 loan record is required.</p>",
                                margin: '0 0 0 20',
                                forceSelection: true,
                                enforceMaxLength: true,
                                readOnly : true,
                    ...
                    },
                    {
                        ...
                        store: {
                            ...
                            type: 'Partner', proxy: {
                                type: 'ajax',
                                url: 'index.php?hdl=partner&action=list&partnercode=KTP',
                                reader: {
                                    type: 'json',
                        ...
                        listeners: {
                            expand: function(combo){
                                ...
                                combo.store.filter("parent", myView.parent);
                            }
                        },
                        // forceSelection: true,
                        // allowBlank: false
        ...
        formPanelItems: [
            //1st hbox
            {
                ...
                items: [
                    ...
                    //             })
                    //             // debugger;
                    //             combo.store.filter("group", 'PKB@UAT');
                    //             combo.store.filter("parent", 1);
                    //         }
                    //     },
                    //     // forceSelection: true,
                    //     // allowBlank: false

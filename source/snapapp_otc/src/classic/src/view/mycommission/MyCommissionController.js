Ext.define('snap.view.mycommission.MyCommissionController', {
    extend: 'snap.view.gridpanel.BaseController',
    alias: 'controller.mycommission-mycommission',
    ...
    getPrintReport: function (btn) {
        ...
        btn.up('grid').getColumns().map(column => {
            if (column.isVisible() && column.dataIndex !== null) {
                ...
                if ('ordpartnername' == column.dataIndex) {
                    // dont push header 
                } else {
        ...
        if (this.getView().nonpeak == true) {
            url = '?hdl=mycommission&action=exportExcel&header=' + header + '&daterange=' + daterange + '&partnercode=' + this.getView().partnercode + '&nonpeak=true';
        } else {
            url = '?hdl=mycommission&action=exportExcel&header=' + header + '&daterange=' + daterange + '&partnercode=' + this.getView().partnercode + '&nonpeak=false';
        }
    ...
    getPrintReportKtp: function (){
        ...
        if(!startDate || !endDate){
            ...
            snap.getApplication().sendRequest({
                hdl: 'mycommission', action: 'getMerchantList', partnercode: myView.partnercode,
...
getPrintReportPkbAffi: function (){
    ...
    GenerateReport: function(selectedID){
        ...
        daterange = encodeURIComponent(JSON.stringify(daterange));
        if (this.getView().nonpeak == true) {
            url = '?hdl=mycommission&action=exportExcel&header=' + header + '&daterange=' + daterange + '&partnercode=' + myView.partnercode + '&nonpeak=true'+'&selected='+selectedID;
        } else {
            url = '?hdl=mycommission&action=exportExcel&header=' + header + '&daterange=' + daterange + '&partnercode=' + myView.partnercode + '&nonpeak=false'+'&selected='+selectedID;
        }
            ...
            function (data) {
                if (data.success) {
                    ...
                    for(i = 0; i < data.merchantdata.length; i++){
                        panel.add({
                            columnWidth:0.5,
                            boxLabel: data.merchantdata[i].name, 
                            name: 'merchant-radiobuttons', 
                            inputValue: data.merchantdata[i].id,
                            uncheckedValue: '0', 
                            reference: data.merchantdata[i].name,
                        });
                    }
    ...
    loadGridTable: function(selectedID){

        header = [];
        // mycommissionview = 'my'+ myView.partnercode.toLowerCase() + 'commissionview';
        if (this.getView().nonpeak == true) {
            url = '?hdl=mycommission&action=list&partnercode=' + myView.partnercode + '&nonpeak=true'+'&selected='+selectedID;
        } else {
            url = '?hdl=mycommission&action=list&partnercode=' + myView.partnercode + '&nonpeak=false'+'&selected='+selectedID;
        }
        myView.getStore().proxy.url = 'index.php' + url;

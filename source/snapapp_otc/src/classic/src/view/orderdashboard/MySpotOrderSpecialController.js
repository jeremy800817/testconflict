Ext.define('snap.view.orderdashboard.MySpotOrderSpecialController', {
    extend: 'snap.view.gridpanel.BaseController',
    alias: 'controller.myspotorderspecial-myspotorderspecial',


    doSpotOrderSpecialSell: function(elemnt) {
        var me = this;
        var myView = this.getView();
        var originType = myView.type;
        ...
        productspotvalue = Ext.getCmp(productspecial);
       ...
       /*--------------------------------------------- Check Backend if requirements are met ----------------------------------------*/
        ...
        acebuyprice = parseFloat(Ext.getCmp(saleacebuyprice).getRawValue()).toFixed(3);
        //If negative value
                            ...
                            if (spotpanelbuytotalvalue.getForm().isValid()) {
                                ...
                                snap.getApplication().sendRequest({
                                    hdl: 'order', action: 'doSpotOrderSpecial', 
                                        buyprice: Ext.getCmp(saleacebuyprice).getRawValue(), 
                                        uuid: Ext.getCmp(saleorderuuid).getRawValue(),
                                        amount: spotorder.totalvalue,
                                        productitem: spotorder.product,
                                        sellorbuy: 'buy',
                                        customerid: Ext.getCmp(customerspecial).getValue(),
                                ...
                                function(data){
                                    if(data.success){
                                        ...
                                        // Clear input form
                                        Ext.getCmp(orderdashboardspotorderspecialform).reset();
        ...
        var windowforspotorderbuyxau = new Ext.Window({
         ...
         /* ---------------------------------- Check Permission for Spot Order Sell --------------------------------- */
                ...
                productpermission = allpermissions.find(x => x.id === spotorder.product && x.partnerid === spotorder.customer);
                ...
                */
                ...
                for (index = 0; index <forminput.length; index++ ){
                    ...
                    if(forminput[index].value != ""){
                        ...
                        // Check id
                        if(forminput[index].id == totalvaluespotspecial || forminput[index].id == totalxauweightspotspecial || forminput[index].id == saleacebuyprice){
                            ...
                            if(forminput[index].id == totalvaluespotspecial && Ext.getCmp(totalvaluespotspecial).disabled == false){
                            ...
                            if(forminput[index].id == totalxauweightspotspecial && Ext.getCmp(totalxauweightspotspecial).disabled == false){
                            ...
                            if(forminput[index].id == saleacebuyprice && Ext.getCmp(saleacebuyprice).disabled == false){
...
`       */
       ...
       // Get orderfees
        ...
        acesellprice = parseFloat(Ext.getCmp(saleacesellprice).getRawValue()).toFixed(3);
            ...
            buttons: [{
                ...
                handler: function(btn) {
                    if (spotpanelselltotalvalue.getForm().isValid()) {
                        ...
                        snap.getApplication().sendRequest({ 
                            hdl: 'order', action: 'doSpotOrderSpecial', 
                                sellprice: Ext.getCmp(saleacesellprice).getRawValue(), 
                                uuid: Ext.getCmp(saleorderuuid).getRawValue(),
                                amount: spotorder.totalvalue,
                                productitem: spotorder.product,
                                sellorbuy: 'sell',
                ...
                */
                ...
                for (index = 0; index <forminput.length; index++ ){
                    ...
                    if(forminput[index].value != ""){
                        ...
                        // If the filled fields are the 4 inputs
                        // Check id
                        if(forminput[index].id == totalvaluespotspecial || forminput[index].id == totalxauweightspotspecial || forminput[index].id == saleacesellprice){
                            ...
                            if(forminput[index].id == saleacesellprice && Ext.getCmp(saleacesellprice).disabled == false){
    ...
    getTransactionReport: function(btn){
        ...
        */
    //    type = btn.reference;
       specialpanel = myView.type + 'specialspotgridpanel';

       specialGridPanel = btn.up().up().up().lookupController().lookupReference(specialpanel);


       const reportingFields = [
            ['Date', ['createdon', 0]], 
            // ['Transaction Ref No', ['partnerrefid', 0]],
       ...
       specialGridPanel.getColumns().map(column => {
        ...
        partnerCode = specialGridPanel.up().up().partnerCode;
        //url = '?hdl=bmmborder&action=exportExcel&header='+header+'&daterange='+daterange+'&type='+type;'
        url = '?hdl=myspotorderspecial&action=exportExcel&header='+header+'&daterange='+daterange+'&partnercode='+partnerCode;

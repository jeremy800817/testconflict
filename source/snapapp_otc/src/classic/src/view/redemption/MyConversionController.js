Ext.define('snap.view.redemption.MyConversionController', {
    ...
    */
    ...
    printConvertAqadFromList: function(btn,myView, gridFormView, partnercode, footer) {
        let me = this;
        let selection = me.getView().getSelectionModel().getSelection()[0];

        var partnerCode = me.getView().partnercode;
        var customerid = selection.get('accountholderid');
        var refno = selection.get('refno');
        var footer = 'bank';

        printType = btn.reference;

        var url = 'index.php?hdl=myconversion&action=printConversionOTC&partnercode='+partnerCode+'&customerid='+customerid+'&refno='+refno+'&footer='+footer+'&printType='+printType;

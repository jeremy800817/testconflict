...
Ext.define('snap.view.redemption.MyConversionRequests', {
    ...
    toolbarItems: [
        ...
        'detail', 'filter','|',
        { reference: 'sendButton', text: 'Send', tooltip: 'Send to Logistics', iconCls: 'x-fa fa-paper-plane', handler: 'scheduleDate', validSelection: 'single', 
        listeners:{
           afterrender: function() {
            this.itemId = this.up().up().partnercode + 'sendToLogistics';
            }
        } },
        { reference: 'approveButton', hidden: true, text: 'Send', tooltip: 'Update Status', iconCls: 'x-fa fa-check-square', handler: 'updateStatus', validSelection: 'single',
        listeners:{
            afterrender: function() {
             this.itemId = this.up().up().partnercode + 'approveConversion';
             }
         } },
        { reference: 'sendMailButton', hidden: true, text: 'Send', tooltip: 'Send email to Partner', iconCls: 'x-fa fa-envelope', handler: 'sendEmail', validSelection: 'single',
        listeners:{
            afterrender: function() {
             this.itemId = this.up().up().partnercode + 'sendMailToPartner';
             }
         } },
        ...
        {
            text: 'Download',tooltip: 'Print Daily Conversion Report',iconCls: 'x-fa fa-print', reference: 'dailyconversionreport', handler: 'getConversionReport',  showToolbarItemText: true, printType: 'xlsx', // printType: pending
        },
        {
        ...
        {
            text: 'Print Confirmation', cls: '', tooltip: 'Print Conversion Confirmation', validSelection: 'single', iconCls: 'x-fa fa-print', reference: 'printConvertAqadFromList', handler: 'printConvertAqadFromList',  showToolbarItemText: true, printType: 'xlsx', // printType: pending
        },
        {
            text: 'Print Receipt', cls: '', tooltip: 'Print Conversion Receipt', validSelection: 'single', iconCls: 'x-fa fa-print', reference: 'printConvertReceiptFromList', handler: 'printConvertAqadFromList',  showToolbarItemText: true, printType: 'xlsx', // printType: pending
        },
    ...
    columns: [
        ...
        { text: 'Branch Code', dataIndex: 'rdmbranchid', hidden:true, minWidth:130, renderer: 'setTextColor',filter: { type: 'string' } },
        { text: 'Branch Name', dataIndex: 'rdmbranchname', hidden:true, minWidth:130, renderer: 'setTextColor',filter: { type: 'string' } },
    ...
    listeners: {
        ...
        afterrender: function () {
            ...
            // check if nubex
            if(this.personalcourier == true){
                // hide courier
                sendToLogistics = this.partnercode+'sendToLogistics';
                approveConversion = this.partnercode+'approveConversion';
                sendMailToPartner = this.partnercode+'sendMailToPartner';

                var btnSendLogistics = Ext.ComponentQuery.query("#" + sendToLogistics)[0];
                var btnChangeStatus = Ext.ComponentQuery.query("#" + approveConversion)[0];
                var btnSendMail = Ext.ComponentQuery.query("#" + sendMailToPartner)[0];
 
                btnSendLogistics.setHidden(true);
                
                // show manual approval
                btnChangeStatus.setHidden(false);

                // show email
                btnSendMail.setHidden(false);
            }
        },

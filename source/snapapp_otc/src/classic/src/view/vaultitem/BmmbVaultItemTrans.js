Ext.define('snap.view.vaultitemtrans.BmmbVaulItemTrans', {
    extend: 'snap.view.vaultitemtrans.vaultitemTrans',
    xtype: 'bmmbvaultitemtransview',
    requires: [
    ...
    store: {
        type: 'VaultItemTrans', proxy: {
            type: 'ajax',
            url: 'index.php?hdl=vaultitemtrans&action=list&partnercode=BMMB',
            reader: {
                type: 'json',
    ...
    checkActionPermission: function (view, record) {
        ...
        var transferPermission = snap.getApplication().hasPermission('/root/bmmb/vault/transfer');
        var returnPermission = snap.getApplication().hasPermission('/root/bmmb/vault/return');

Ext.define('snap.view.vaultitemtrans.BsnVaulItemTrans', {
    extend: 'snap.view.vaultitemtrans.vaultitemTrans',
    xtype: 'bsnvaultitemtransview',
    requires: [
    ...
    store: {
        type: 'VaultItemTrans', proxy: {
            type: 'ajax',
            url: 'index.php?hdl=vaultitemtrans&action=list&partnercode=BSN',
            reader: {
                type: 'json',
    ...
    checkActionPermission: function (view, record) {
        ...
        var transferPermission = snap.getApplication().hasPermission('/root/bsn/vault/transfer');
        var returnPermission = snap.getApplication().hasPermission('/root/bsn/vault/return');

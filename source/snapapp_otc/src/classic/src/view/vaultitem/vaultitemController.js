Ext.define("snap.view.vaultitem.vaultitemController", {
  ...
  showTransferForm: function (record) {
    ...
    // No physical bar
    if (
      selectedRecords[0].data.status == 1 &&
      selectedRecords[0].data.allocated == 1 &&
      selectedRecords[0].data.vaultlocationid == 0
    ) {
      var defaultvalue = "1";
      var fromlocation = Ext.create("Ext.data.Store", {
        ...
        data: [
          {
            id: selectedRecords[0].data.vaultlocationid,
            name: "ACE HQ",
            value: defaultvalue,
          },
        ],
      ...
      var tolocation = {
        ...
        proxy: {
          type: "ajax",
          url:
            "index.php?hdl=vaultitem&action=getTransferLocationsStart&origintype=" +
            origintype,
          reader: {
            type: "json",
    ...
    if (
      selectedRecords[0].data.status == 1 &&
      selectedRecords[0].data.allocated == 1 &&
      selectedRecords[0].data.vaultlocationid == 1
    ) {
      ...
      var fromlocation = Ext.create("Ext.data.Store", {
        ...
        data: [
          {
            id: selectedRecords[0].data.vaultlocationid,
            name: selectedRecords[0].data.vaultlocationname,
            value: defaultvalue,
          },
        ],
      });
    ...
    if (
      selectedRecords[0].data.status == 1 &&
      selectedRecords[0].data.allocated == 1 &&
      selectedRecords[0].data.vaultlocationid == 2
    ) {
      var defaultvalue = selectedRecords[0].data.vaultlocationid;
      var fromlocation = Ext.create("Ext.data.Store", {
        fields: ["id", "name"],
      ...
      var tolocation = {
        ...
        proxy: {
          type: "ajax",
          url:
            "index.php?hdl=vaultitem&action=getTransferLocationsIntermediate&origintype=" +
            origintype,
          reader: {
            type: "json",
            rootProperty: "locations",
    ...
    if (
      (selectedRecords[0].data.status == 1 &&
        (selectedRecords[0].data.allocated == 0 ||
          selectedRecords[0].data.allocated == 1) &&
        selectedRecords[0].data.vaultlocationid == 3) ||
      selectedRecords[0].data.vaultlocationid == 4
    ) {
      var defaultvalue = selectedRecords[0].data.vaultlocationid;
      var fromlocation = Ext.create("Ext.data.Store", {
        fields: ["id", "name"],
      ...
      var tolocation = {
        ...
        proxy: {
          type: "ajax",
          url:
            "index.php?hdl=vaultitem&action=getTransferLocationsEnd&origintype=" +
            origintype,
          reader: {
            type: "json",
            rootProperty: "locations",
    ...
    for (i = 0; i < selectedRecords.length; i++) {
      ...
      if (length <= 1) {
        serialNumberLine +=
          ...
          count +
          ".&nbsp;" +
          selectedRecords[i].data.serialno +
          "&nbsp;".repeat(25);
      } else {
        // Reset length
        length = 0;
    ...
    layout =
      "<div style='height:100px;width:350px;border:1px solid #4e4e4e;overflow:auto;'>" +
      selectedSerialNumbers +
      "</div>";
      ...
      buttons: [
        {
          ...
          handler: function (btn) {
            if (transferpanel.getForm().isValid()) {
              ...
              vaultFrom =
                this.up().up().items.items[0].items.items[0].items.items[0]
                  .items.items[0].value;
              vaultTo =
                this.up().up().items.items[0].items.items[0].items.items[1]
                  .items.items[0].value;
              ...
              transferpanel.getForm().submit({
                ...
                params: {
                  hdl: "vaultitem",
                  action: "requestForTransferItemMultiple",
                  "serialno[]": serialno,
                  vaultfrom: vaultFrom,
                  vaultto: vaultTo,
                },
                ...
                success: function (frm, action) {
                  ...
                  myView
                    ...
                    .lookupReference("summarycontainer")
                    .doFireEvent("reloadsummary");

                  snap
                    .getApplication()
                    .getStore("snap.store.VaultItemTrans")
                    .reload();
                },
  ...
  showTransferFormCommon: function (record) {
    ...
    if (
      selectedRecords[0].data.status == 2 &&
      (selectedRecords[0].data.allocated == 0 ||
        selectedRecords[0].data.allocated == 1) &&
      selectedRecords[0].data.vaultlocationid == 1
    ) {
  ...
  requestTransfer: function (record) {
    ...
    Ext.MessageBox.confirm(
      "Confirm",
      "Are you sure you want to transfer item ?",
      function (btn) {
        if (btn === "yes") {
          snap
            .getApplication()
            .sendRequest(
              {
                ...
                serialno: selectedRecords[0].data.serialno,
              },
              "Sending request...."
            )
            .then(function (data) {
  ...
  myRequestTransfer: function (record) {
    ...
    if (startLocation == selectedRecords[0].data.vaultlocationid) {
      messageText =
        "Are you sure you want to transfer the following to BMMB? : \n" +
        layout;
      vaultTo = endLocation;
    } else {
      messageText =
        "Are you sure you want to transfer item the following to ACE HQ? : \n" +
        layout;
      vaultTo = startLocation;
    }
    Ext.MessageBox.confirm("Confirm", messageText, function (btn) {
      if (btn === "yes") {
        snap
          .getApplication()
          .sendRequest(
            {
              hdl: "vaultitem",
              action: "requestForTransferItemMultipleConfirmation",
              "serialno[]": serialno,
  ...
  cancelTransfer: function (record) {
    ...
    Ext.MessageBox.confirm(
      "Confirm",
      "Are you sure you want to cancel transfer for the following items? \n" +
        layout,
      function (btn) {
        if (btn === "yes") {
          snap
          ...
          .sendRequest(
            {
              ...
              "serialno[]": serialno,
              isbmmbvaultbalance: isbmmbvaultbalance,
              ismib: ismib,
            },
            "Sending request...."
          )
            ...
            if (data.success) {
              ...
              if (
                ...
                if (startLocation == selectedRecords[0].data.vaultlocationid) {
                  ...
                  Ext.get(reservedcount).dom.innerHTML =
                    parseInt(Ext.get(reservedcount).dom.innerHTML) - count;
                  Ext.get(g4scount).dom.innerHTML =
                    parseInt(Ext.get(g4scount).dom.innerHTML) + count;

                  Ext.get(vaultamount).dom.innerHTML = data.balance.vaultamount;
                  Ext.get(totalcustomerholding).dom.innerHTML =
                    data.balance.totalcustomerholding;
                  Ext.get(totalbalance).dom.innerHTML =
                ...
                .then(function (data) {
                  if (data.success) {
                    ...
                    if (
                      ...
                      // Check and modify visuals
                      if (
                        startLocation == selectedRecords[0].data.vaultlocationid
                      ) {
                        // If Transfer is made from ace -> destination not here do it in confirm
              ...
              if (data.success) {
                ...
                if (
                  ...
                  if (1 == selectedRecords[0].data.vaultlocationid) {
                    ...
                    Ext.get(vaultamount).dom.innerHTML =
                      data.balance.vaultamount;
                    Ext.get(totalcustomerholding).dom.innerHTML =
                      data.balance.totalcustomerholding;
                    Ext.get(totalbalance).dom.innerHTML =
                      data.balance.totalbalance;
  ...
  requestActivateItemForTransfer: function (record) {
    ...
    // If starting location is ace
    messageText =
      "Are you sure you want to allocate the following? : \n" + layout;
      ...
      if (btn === "yes") {
        snap
          ...
          .sendRequest(
            {
              hdl: "vaultitem",
              action: "requestActivateItemForTransfer",
              "serialno[]": serialno,
            },
            "Sending request...."
          )
  ...
  approvePendingItemForTransfer: function (record) {
    ...
    // If starting location is ace
    messageText =
      "Are you sure you want to approve the allocation for the following? : \n" +
      layout;
      ...
      if (btn === "yes") {
        snap
          ...
          .sendRequest(
            {
              hdl: "vaultitem",
              action: "approvePendingItemForTransfer",
              "serialno[]": serialno,
            },
            "Sending request...."
          )
  ...
  printButton: function (btn) {
    ...
    var selectedRecords = sm.getSelection();
    var address =
      selectedRecords[0].data.address1 +
      " " +
      selectedRecords[0].data.address2 +
      " " +
      selectedRecords[0].data.address3;

    var record = selectedRecords[0].data;

    var url =
      "index.php?hdl=vaultitem&action=getPrintDocuments&id=" + record.id;
    Ext.Ajax.request({
      url: url,
      method: "get",
      waitMsg: "Processing",
  ...
  createPrintButton: function (btn, formAction) {
    ...
    var gridFormView = Ext.create(
      myView.formClass,
      Ext.apply(myView.createvaultdocument ? myView.createvaultdocument : {}, {
        formDialogButtons: [
          {
            xtype: "panel",
            flex: 1,
    ...
    if ("myvaultitemview" == btn.up().up().xtype) {
      combobox =
        this.gridFormView.items.items[0].items.items[0].items.items[0].items
  ...
  printDocumentSelection: function (record) {
    ...
    var address =
      selectedRecords[0].data.deliveryaddress1 +
      " " +
      selectedRecords[0].data.deliveryaddress2 +
      " " +
      selectedRecords[0].data.deliveryaddress3 +
      " " +
      selectedRecords[0].data.deliverystate;
            ...
            },
            {
              ...
              store: {
                ...
                proxy: {
                  type: "ajax",
                  url:
                    "index.php?hdl=vaultitem&action=getTransferLocationsAll&partnercode=" +
                    partnerCode,
                  reader: {
                    type: "json",
                    rootProperty: "locations",
        ...
        buttons: [
          {
            ...
            handler: function (btn) {
              if (schedulepanel.getForm().isValid()) {
                ...
                var url =
                  "index.php?hdl=vaultitem&action=getPrintDocuments&id=" +
                  record.id +
                  "&from=" +
                  from +
                  "&to=" +
  ...
  onPreAddEditSubmit: function (formAction, theGridFormPanel, theGridForm) {
    ...
    polists = theGridFormPanel
      .getController()
      .lookupReference("pocontainer")
      .getStore().data.items;
    scheduledate = theGridFormPanel
      .getController()
      .lookupReference("scheduledate");
    ...
    customer = theGridFormPanel
      .getController()
      .lookupReference("documentcombox").value;
    req = {
      po: trans_po,
      type: customer,
    };
  ...
  exportVaultListButton: function (btn) {
    ...
    url =
      "?hdl=vaultitem&action=exportVaultList&header=" +
      header +
      "&daterange=" +
      daterange +
      "&partnercode=" +
    ...
    snap
      ...
      .then(function (data) {
        if (data.success) {
          ...
          Ext.get("mibtransferringcount").dom.innerHTML =
            data.transferringcount;
  ...
  getdatefilter: function () {
    ...
    // Apply the date filter to the store
    var store = this.getView().getStore();
    store.clearFilter();
    store.filterBy(function (record) {
        var recordDate = record.get('createdon'); // Replace 'createdon' with the appropriate date field name
        return (recordDate >= startDate && recordDate <= endDate);
    });
  },

  // Handler for the "Clear Date" button
  cleardatefilter: function () {
    ...
    // Clear the date filter from the store
    var store = this.getView().getStore();
    store.clearFilter();
  },
  
  getinventoryreport: function (btn) {
    header = [];
    btn
      .up("grid")
      .getColumns()
    ...
    url =
      "?hdl=vaultitem&action=exportexcel2&header=" +
      header +
      "&daterange=" +
      daterange +
      "&partnercode=" +
  ...
  OrderGoldStore: function (btn) {
    let me = this;
    let selection = me.getView();

    let win = new Ext.Window({
        title: 'Order Gold from Ace',
        layout: 'center', // Center the form within the window
        width: 530,
        closeAction: 'close',
        
        items: [{
            xtype: 'form', // Use a form as the main container
            layout: 'form',
            bodyPadding: 10,
            items: [{
                xtype: 'textfield',
                fieldLabel: 'Gold Weight (Kg)',
                emptyText: 'eg. 1',
                value: '',
                itemId: 'goldweight',
                allowBlank: false, // Set as required field
                maskRe: /[0-9]/, // Restrict input to numbers only
        ...
        buttons: [{
            ...
            //     // Ajax to handler
            //     snap.getApplication().sendRequest({
            //         hdl: 'myvaultitem', action: 'makeOrderInventory', id: data.id,
            //     }, 'Fetching data from server....').then(
            //     //Received data from server already
            //     function(data){
            //         if(data.success){

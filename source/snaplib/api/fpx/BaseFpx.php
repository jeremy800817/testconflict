...
abstract class BaseFpx implements IFpxOperation, IObservable
{
    use TLogging;
    use TObservable;
    ...
    final static public function getInstance($handlerName) {
        $matches = [];
        if (preg_match('/^\\\\Snap\\\\api\\\\fpx\\\\(.*)$/',$handlerName, $matches)) {
        ...
        if(class_exists($className)) {
            ...
            $app->logDebug("Instantiating FPX handler $className");
    ...
    /**
     * Verify signed data received from FPX
     * 

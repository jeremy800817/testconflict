 ...
 */
class GtpApiProcessor1_0b extends GtpApiProcessor
{
    /**
    ...
    public function process($app, \Snap\api\param\ApiParam $apiParam, $decodedData, $requestParams)
    {
        switch($apiParam->getActionType()) {
            ...
            case 'query_price_stream':
                ...
                break;
            case 'getlogisticstatus':
                return $this->doGetLogisticStatus($app, $apiParam, $decodedData, $requestParams);
                break;
    ...
    {
        try {
            $productforKilobar = 'GS-999-9-1000g';
            $product = $app->productStore()->getByField('code', $productforKilobar);
            $version = strtolower($requestParams['version']);
    ...
    private function doRedemption($app, \Snap\api\param\ApiParam $apiParam, $decodedData, $requestParams)
    {
        try {
            ...
            if($responseParams['status'] == '1'){
                // send email
                $receiver = 'accounts@ace2u.com,ridwan@bursamalaysia.com';
        
                $developmentEnv = $app->getConfig()->{'snap.environtment.development'};
                if ($developmentEnv){
                    $receiver = 'rinston@silverstream.my';
                }
                
                
                $subject = 'BursaGold Conversion Request';
                $body = '
                <img class="img-fluid" style="width: 750px;height: 303px;" src="https://gtp2.ace2u.com/src/resources/images/BURSA/conversion.png" alt="BURSA Conversion"><br><br>
                Dear accounts@ace2u.com,
                <br>
                ...
                <br>
                Date requested: '.$redemptionValidation->createdon->format('Y-m-d h:i:s').'<br>
                Reference Number: '.$redemptionValidation->redemptionno.'<br>
                Product Weight: '.$redemptionValidation->totalweight.'<br>
                Quantity: '.$redemptionValidation->totalquantity.'<br>
                <br><br>
                Item(s): 
                ';

                $items = $redemptionValidation->items;
                foreach (json_decode($items) as $x => $item){
                    $y= $x+1;
                    $body .= 'no.'.$y.' - '.$item->code.' - '.$item->serialnumber.'<br>';
                }

                $deliveryaddress = $redemptionValidation->deliveryaddress1.",".$redemptionValidation->deliveryaddress2.",".$redemptionValidation->deliverypostcode." ".$redemptionValidation->deliverystate;
                $body .= '<br><br>
                Name: '.$redemptionValidation->deliverycontactname1.'<br>
                Address: '.$deliveryaddress.'<br>
                Contact No: '.$redemptionValidation->deliverycontactno1.'<br>
                <br><br>
    ...
    private function doGetLogisticStatus($app, \Snap\api\param\ApiParam $apiParam, $decodedData, $requestParams){
        try {
            $partner = $decodedData['partner'];

            $redemption = $app->redemptionStore()->searchTable()->select()->where('redemptionno', $requestParams['ref_id'])->andWhere('partnerid', $partner->id)->one();
            if(!$redemption){
                throw new \Exception("Redemption data for id: {$requestParams['ref_id']} not found");
            }

            $logistic = $app->logisticStore()->searchTable()->select()->where('typeid', $redemption->id)->andWhere('type', 'Redemption')->one();
            if(!$logistic){
                throw new \Exception("Logistic data for id: {$requestParams['ref_id']} not found");
            }

            $vendorValue = $app->tagStore()->getById($logistic->vendorid);

            if ($vendorValue->value == \Snap\object\Logistic::VENDOR_GDEX_VALUE){
                $logistic_status = $app->logisticManager()->gdexApiGetShipmentStatusDetail($logistic);
            }
            // if ($vendorValue->value == Logistic::VENDOR_LINCLEAR_VALUE){
            //     $return = $app->logisticManager()->lineclearApiGetShipmentStatusDetail($logistic);
            // }
            if ($vendorValue->value == \Snap\object\Logistic::VENDOR_ACEDELIVERY_VALUE || $vendorValue->value == \Snap\object\Logistic::VENDOR_JNT_VALUE){
                $logistic_status = $app->logisticManager()->aceDeliveryGetShipmentStatusDetail($logistic);
            }

            // $logistic_status = $app->logisticManager()->gdexApiGetShipmentStatusDetail($logistic);
            
            $decodedData['logistic'] = $logistic;
            $decodedData['logstatus'] = $logistic_status;
            // $decodedData['logstatus'] = json_encode($logistic_status);

            $responseParams = $this->createOutputApiParam('getlogisticstatusResponse', $requestParams, $decodedData);
            $sender = \Snap\api\gtp\GtpApiSender::getInstance('Json', $responseData);
            $sender->response($app, $responseParams);

            \Snap\App::getInstance()->logDebug("doGetLogisticStatus()_response");

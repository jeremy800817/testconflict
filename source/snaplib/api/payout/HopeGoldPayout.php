...
class HopeGoldPayout extends BasePayout 
{
    ...
    protected function __construct($app)
    {
        ...
        if (! $this->merchantId) {
            throw new \Exception("HopeGoldPayout merchant ID not provided");
        }
    }
    ...
    {
        try {
            //echo "Come to HopeGold payout \n";
            $this->log("------ HopeGoldPayout credit customer request -----", SNAP_LOG_DEBUG);
            $response = $this->creditCustomer($disbursement);
            $this->log("------ HopeGoldPayout credit customer request END -----", SNAP_LOG_DEBUG);
            
            if (200 == $response->getStatusCode()) {
                $response = $response->getBody()->getContents();
                $this->log("------ HopeGoldPayout credit customer response -----", SNAP_LOG_DEBUG);
                $this->log($response, SNAP_LOG_DEBUG);
                $responseArr = json_decode($response, true);                
                $this->processDisbursementResponse($disbursement, $responseArr);
                $this->log("------ HopeGoldPayout credit customer response END -----", SNAP_LOG_DEBUG);
            }
            ...
            $this->log("--- HopeGoldPayout Credit Customer Error Start ---", SNAP_LOG_ERROR);
            ...
            $this->log("--- HopeGoldPayout Credit Customer Error End ---", SNAP_LOG_ERROR);
            return false;
        }
    }
    ...
    protected function processDisbursementResponse($disbursement, $responseArr)
    {
        ...
        if (self::STATUS_SUCCESS === strtolower($responseArr['status'])) {
            ...
            $this->log(__METHOD__ . ": Failed payment response received for HopeGoldPayout {$disbursement->refno} - {$disbursement->accountnumber}" , SNAP_LOG_ERROR);
    ...
    public function getPayoutStatus($disbursement)
    {
        try {     
            ...
            $this->log("---HopeGoldPayout wallet - Start send request to wallet to checking settlement status for  ".$plainData."---", SNAP_LOG_DEBUG);
            ...
            $this->log("---HopeGoldPayout checking payout response json ".$plainData."---", SNAP_LOG_DEBUG);
            //Array success
            //(
            //    [status] => success
            ...
            if (self::STATUS_SUCCESS === strtolower($returnArray['status']) && array_key_exists('data', $returnArray)) {
                $this->log("------ HopeGoldPayout transaction payout status found for {$disbursement->transactionrefno} -----", SNAP_LOG_DEBUG);
                $this->processCallback($returnArray['data']);
                return $returnArray;
            }
            $this->log("------ HopeGoldPayout transaction payout status not found for {$disbursement->transactionrefno} -----", SNAP_LOG_ERROR);
            return $returnArray;
        } catch (\GuzzleHttp\Exception\RequestException $e) {
            $body = $e->getResponse()->getBody()->getContents();
            $this->log("--- HopeGoldPayout Get payout Transaction Status Error Start ---", SNAP_LOG_ERROR);
            $this->log(sprintf("Error code: %d", $e->getCode()), SNAP_LOG_ERROR);
            $this->log(sprintf("URL: %s", $e->getRequest()->getUri()->getHost().$e->getRequest()->getRequestTarget()), SNAP_LOG_ERROR);
            $this->log($body, SNAP_LOG_ERROR);
            $this->log("--- HopeGoldPayout Get payout Transaction Status Error End ---", SNAP_LOG_ERROR);
        }
    }

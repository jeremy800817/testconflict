 ...
 */
class SapApiProcessor implements IApiProcessor
{
    use \Snap\TLogging;
     ...
     * @return SapApiProcessor derived class
     */
    static function getInstance($version)
    {
        $className = __CLASS__ . preg_replace('/\./', '_', $version);
        \Snap\App::getInstance()->log("Instantiating Sap processor $className...", SNAP_LOG_ERROR);
    ...
    {
        if (preg_match('/[0-9\.]+m$/', $order->apiversion) || preg_match('/[0-9\.]+p$/', $order->apiversion)) { //digital gold
    ...
    {
        if (preg_match('/[0-9\.]+m$/', $order->apiversion)) { //digital gold
    ...
    {
        if (preg_match('/[0-9\.]+m$/', $redemption->apiversion)) { //digital gold
    ...
    {
        if (preg_match('/[0-9\.]+m$/', $buyback->apiversion)) { //digital gold
    ...
    public function notifyReconcile($app, $transaction){
        if(preg_match('/[0-9\.]+m$/', '1.0m')) {
            ...
            foreach($action as $aAction){
                ...
                $responseParams = $this->createOutputApiParam($aAction, ['version' => '1.0m'], $decodedData);
                $sender = \Snap\api\sap\SapApiSender::getInstance('http', $responseData);
    ...
    public function notifyUpdateReplenish($app, $transaction,$version,$flag)
    {
        ...
        if(preg_match('/[0-9\.]+m$/', $version)) {
            ...
            */
            ...
            $sheet->setCellValue('A2', 'Comparison between MIB and SAP inventory');
            ...
            $sheet2->setCellValue('A2', 'Comparison between MIB and SAP inventory');
            ...
            $sheet3->setCellValue('A2', 'Comparison between MIB and GTP inventory after SAP request is successful');
            ...
            $sheet4->setTitle('MIB INVENTORY SUMMARY');
            ...
            */
            ...
            if(count($kilobarList) > 0){
                foreach($kilobarList as $aKilobarList){
                    ...
                    $decodedData['whsCode'] = 'MIB_VLT';
                    $decodedData['customerId'] = 'MIB';
            ...
            $sheet->mergeCells('A2:I2');
            $sheet->setCellValue('A2', 'Comparison between MIB and SAP inventory. Noted: Buyback minted does not trigger SAP when process PHYRTN ftp.');
    ...
    public function notifyNewReserveSerialNum($app, $redemption)
    {
        if (preg_match('/[0-9\.]+m$/', $redemption->apiversion)) { //digital gold
            ...
            foreach ($items as $x => $aItem) {
                ...
                $aRedemption['quantity']    = (float)$aItem->denomination;
                $aRedemption['whsCode']     = 'MIB_BNK';
                $aRedemption['bankId']      = $redemption->branchid;
                $aRedemption['customerId']  = 'MIB';
    ...
    public function notifyNewUnreserveSerialNum($app, $redemption)
    {
        if (preg_match('/[0-9\.]+m$/', $redemption->apiversion)) { //digital gold
    ...
    {
        $outputApiParam = \Snap\api\param\SapApiParam::getInstance($requestParams['version']);
    ...
    private function sapReturnVerify($sap_response,$filename){
        $response_data = $sap_response['data'];
        // very array data;
        $this->log("{$filename} - sapReturnVerify - verify sap return on item status :" . gmdate('Y-m-d H:i:s') , SNAP_LOG_DEBUG);
        if ($response_data && 'N' == $response_data[0]['success']){
            return false;
        }
        if (!$response_data){
            return false;
        }
        foreach ($response_data as $sap_data){

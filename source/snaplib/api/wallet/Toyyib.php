...
class Toyyib extends BaseWallet
{
    ...
    {        
        $this->log("------ Toyyib prepare transaction payment location -----", SNAP_LOG_DEBUG);
        ...
        $this->log("------ Toyyib prepare transaction payment location END -----", SNAP_LOG_DEBUG);
        $jsonEncodedRequestData = json_encode($data);
        $this->logApiRequest($jsonEncodedRequestData, $payment);

        /*send payment to toyyib wallet*/
    ...
    {   
        $this->log("------ Start sending transaction to Toyyib wallet -----", SNAP_LOG_DEBUG);
        try {
            ...
            $this->log("------ End sending transaction to Toyyib wallet -----", SNAP_LOG_DEBUG);
    ...
    {
        $this->log("------ Processing toyyib payment response -----", SNAP_LOG_DEBUG);
        $plainData = json_encode($responseArr);

        $this->log("---Toyyib response json ".$plainData."---", SNAP_LOG_DEBUG);
        //Array
        //(
        //    [status] => success
        //    [message] => Wallet transaction success
        ...
        if (self::STATUS_SUCCESS === strtolower($responseArr['status'])) {
            ...
            $this->log("---Toyyib wallet - Payment {$payment->paymentrefno} response received but no action was taken.", SNAP_LOG_ERROR);
            $this->handleError($responseArr['message'],$payment);
        }

        $this->log("------ Processing toyyib payment response END -----", SNAP_LOG_DEBUG);
    }
    ...
    /**
     * Get status of transaction from Toyyib
     * 
     * @param MyPaymentDetail $paymentDetail
     * 
     */
    ...
    {   
        try {     
            ...
            $returnArray = json_decode($response->getBody()->getContents(), true);
            $plainData = json_encode($returnArray);
            $this->log("---Toyyib wallet - Start send request to wallet to checking settlement status for  ".$plainData."---", SNAP_LOG_DEBUG);
            $this->log("---Toyyib wallet - checking response json ".$plainData."---", SNAP_LOG_DEBUG);
            ...
            if (self::STATUS_SUCCESS === strtolower($returnArray['status']) && array_key_exists('data', $returnArray)) {
                $this->log("------ Toyyib transaction status found for {$paymentDetail->sourcerefno} -----", SNAP_LOG_DEBUG);
                $this->processCallback($returnArray,$paymentDetail,$customertype);
                return $returnArray;
            }
            $this->log("------ Toyyib transaction status not found for {$paymentDetail->sourcerefno} -----", SNAP_LOG_ERROR);
            return $returnArray;
        } catch (\GuzzleHttp\Exception\RequestException $e) {
            $body = $e->getResponse()->getBody()->getContents();
            $this->log("--- Toyyib Get Transaction Status Error Start ---", SNAP_LOG_ERROR);
            ...
            $this->log("--- Toyyib Get Transaction Status Error End ---", SNAP_LOG_ERROR);
        }
    }
    
    /**
     * Verifies the callback from Toyyib
     * 
     * @return boolean
     */
    ...
    {
        $this->log("---Toyyib wallet - Process success settlement to update GTP pending transaction ".$payment->sourcerefno."---", SNAP_LOG_DEBUG);
            //[data] => Array
            //        (
            //            [transaction_ref_no] => PAY567469202232595550
            //            [transaction_amount] => 5.27
        ...
        $toyyibStatus = strtolower($response['data']['transaction_status']);
        $now = new \DateTime();
        $now->setTimezone($this->app->getUserTimezone());
        ...
        $this->logDebug(__METHOD__."(): Received checking response from Toyyib");
        ...
        $createLedger = false;
        if (self::STATUS_FAILED == $toyyibStatus && $state->can(MyPaymentDetail::STATUS_FAILED)) {
            $this->log("---Toyyib wallet - Settlement status return failed ".$payment->sourcerefno."---", SNAP_LOG_DEBUG);
            $action = IObservation::ACTION_REJECT;
            $payment->status = MyPaymentDetail::STATUS_FAILED;
            $payment->failedon = $now;
            $goldTx->status = MyGoldTransaction::STATUS_FAILED;
        } else if (self::STATUS_SUCCESS == $toyyibStatus) {
            $this->log("---Toyyib wallet - Settlement status return success ".$payment->sourcerefno."---", SNAP_LOG_DEBUG);
            $action = IObservation::ACTION_CONFIRM;
            $payment->status = MyPaymentDetail::STATUS_SUCCESS;
            $payment->successon = $now;
            ...
            $this->log("---Toyyib wallet - Payment {$payment->sourcerefno} callback checking received but no action was taken.", SNAP_LOG_ERROR);
            $skip = true;
        }
        ...
        if (!$skip) {
            ...
            if($createLedger) {
                $this->log("---Toyyib wallet - Create ledger for {$payment->sourcerefno} if success / createledger is true.", SNAP_LOG_ERROR);
    ...
    protected function httpClientFactory($addToken = true)
    {
        ...
        $this->log("---Toyyib wallet - Header request send to Toyyib wallet => ".$plainData, SNAP_LOG_DEBUG);

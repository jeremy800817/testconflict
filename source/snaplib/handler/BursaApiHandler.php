...
USe Snap\App;
/**
 * Handler class for all Bursa API to GTP.
 *
 * @author Devon <devon@silverstream.my>
 * @version 1.0
 */
class BursaApiHandler implements \Snap\IHandler {
    private $app = null;
    Use \Snap\TLogging;
    ...
    public function doAction($app, $action, $params)
    {
        ...
        $this->log("----BURSA GTP-processGtpRequest - Start --request ", SNAP_LOG_DEBUG, $params);
        ...
        $this->log("----BURSA GTP-processGtpRequest - End --time({".$timediff."})", SNAP_LOG_DEBUG);
        ...
        if (floatval($timediff) >= 14 && !$dev){
            ...
            $this->log("----BURSA GTP-processGtpRequest - timeout_condition", SNAP_LOG_DEBUG);
            ...
            if ($return['action_requested'] == 'spot_acebuy' || $return['action_requested'] == 'spot_acesell'){
                // fail the order
                $this->log("----BURSA GTP-processGtpRequest - timeout_condition fail OrderNo(".$return['order_id'].")", SNAP_LOG_DEBUG);
            ...
            $mailer->Subject = 'BURSA GTP - slow request - duration - '.$timediff. ' at '.$_now->format('Y-m-d H:i:s');

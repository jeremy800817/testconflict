...
/**
 * Encapsulates the ordercancel table on the database
 *
 * This class encapsulates the ordercancel table data
 * information
 *
 * Data members:
 ...
 */
class OrderCancel extends SnapObject {
  ...
  const QTYPE_GOODTILLCANCEL = 'GoodTillCancel';
    ...
    /**
     * This is the main method to get cancelled order objects that is related to the order.
     *
     */
/*    public function getOrderCancel() {
        if(0 != $this->members['id'] && 0 == count($this->orderdelivery)) {
            $selectHandle = $this->getStore()->getRelatedStore('orderdelivery')->searchTable()->select()
                                            ->where('orderid', $this->members['id'])
                                            ->andWhere('status', SnapObject::STATUS_ACTIVE)
                                            ->orderby('expecteddeliveryon');

            $this->orderdelivery = $selectHandle->execute();
        }
    ...
    /**
     * This is the main method to get cancelled order objects that is related to the order.
     * @return orderdelivery[] Returns all the order delivery objects in array
     *//*
    public function getOrderCancel() {

            $selectHandle = $this->>ordercancelStore()->searchTable()->select()
                                            ->where(['id' => 1]);
                                            //->where('orderid', $this->members['id'])
                                            //->andWhere('status', SnapObject::STATUS_ACTIVE)
                                            //->orderby('id', desc);

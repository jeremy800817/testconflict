...
class otccontroller extends Controller
{
    ...
    */
    public function initialiseController($app, $contextOrMode)
    {
        ...
        if(!$cacheHandle instanceof \Snap\redisarraycacher) {
            throw new Exception("OtcController::initialiseController() unable to continue because it requires Redis cacher to run");
        }
        ...
        $gtpStores = [
            ...
            'mymonthlystoragefee'  => ['class' => '\Snap\store\dbdatastore', 'publicAccessible' => true, 'parameters' => [
                    $dbHandle, 'mymonthlystoragefee', null, 'msf', '\Snap\object\MyMonthlyStorageFee', array('vw_mymonthlystoragefee', 'vw_mymanagementfee', 'vw_myoutstandingmanagementfee'),  array(), true
            ]],
            ...
            'otcuseractivitylog' => [ 'class' => '\Snap\store\dbdatastore', 'publicAccessible' => true,  'parameters' => [
                        $dbHandle, 'otcuseractivitylog', null, 'ual', '\Snap\object\OtcUserActivityLog', array('vw_otcuseractivitylog'), array(), true]],
            'otcregisterremarks' => [ 'class' => '\Snap\store\dbdatastore', 'publicAccessible' => true,  'parameters' => [
                        $dbHandle, 'otcregisterremarks', null, 'orr', '\Snap\object\OtcRegisterRemarks', array(), array(), true]],            
            ...
            'usradditionaldata' => [ 'class' => '\Snap\store\dbdatastore', 'publicAccessible' => true,  'parameters' => [
                        $dbHandle, 'usradditionaldata', null, 'uad', '\Snap\object\UsrAdditionalData', array(),  array(), true]],
        ...
        //Manager configurations for My Modules -IMPORTANT:  Register all observable targets first, then only the observers
        ...
        $this->registerManager('audit', '\Snap\manager\AuditManager', array());
    ...
    */
    public function getAvailableApplicationPermission()
    {
        ...
        // Add super admin by appending user permissions
        '/root/system/user/superadmin' => gettext('Gives superadmin access for user'),
        "/root/developer" => gettext("Developer access only modules"),
        "/root/developer/tag" => gettext("Tag Editing access only modules"),
        "/root/developer/tag/list" => gettext("View Tag Editing access only modules"),
        "/root/developer/tag/add" => gettext("Add Tag Editing access only modules"),
        "/root/developer/tag/edit" => gettext("Edit Tag Editing access only modules"),
        "/root/operator"  => gettext("Operator special access modules"),
		"/root/system/checker"  => gettext("OTC Checker"),
        ...
        //     // extend mygtp on otccontroller for some bo config controller diff
